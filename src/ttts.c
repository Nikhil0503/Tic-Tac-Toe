    ...
    if(numBars == 3 && buf[numBytes-2] == '|'){
      ...
      if(numBars == 4 && buf[numBytes-2] == '|'){
        ...
        if(x == 6){
           ...
           if(strcmp(token, "X") == 0 || strcmp(token, "O") == 0){//if third token is X or O
           ...
           // numBytes-=2;
              ...
              // printf("Token 4: %s", token);
               ...
               if(length == 3){
                if(token[1] == ',' && (token[0] == '1' || token[0] == '2' || token[0] == '3') && (token[2] == '1' || token[2] == '2' || token[2] == '3')){
                  //valid
                 //printf("Hello");
                              return 1;
       ...
       //if second token is a size length
         //if second token using atoi == 6
           ...
           //if third token == "X" or third token == "O"
               //if length of fourth token is 3 and first char is 1-9, second char is comma and third char is 1-9
                   //valid
   ...
   //else if token1 == RSGN
      ...
      if(numBars == 3 && buf[numBytes-2] == '|'){
        ...
        if(x == 2){
           token = strtok_r(buf, "|", &buf);//get third token
           //printf("Hello");
           if(strcmp(token, "S") == 0 || strcmp(token, "A") == 0 || strcmp(token, "S") == 0){
...
void *read_data(void *arg) {
  ...
  while (active && (bytes = read(client ->socket, buf, BUFSIZE)) > 0) {
    ...
    printf("Buf array%s", buf);

    int x = valid(buf, bytes);
    ...
    if(x == 0){
      ...
      if(strcmp(token, "PLAY") == 0){
        memcpy(buffer, "WAIT", 5);
      }

      else if(strcmp(token, "MOVE") == 0){
        memcpy(buffer, "MOVD", 5);
      }


      else if(strcmp(token, "RSGN") == 0){
        memcpy(buffer, "OVER", 5);
      }
